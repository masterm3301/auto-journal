...
const AUTO_CATEGORIES = CATEGORIES.filter((c) => !("manual" in c && c.manual));
...
أجب حصريا بكائن JSON صالح بهذه البنية بالضبط (بدون أي نص خارج الكائن):
{
  ...
  "category": "أحد هذه الرموز فقط: ${AUTO_CATEGORIES.map((c) => c.slug).join(" | ")} — ${AUTO_CATEGORIES.map((c) => `${c.slug}=${c.name}${"focus" in c ? ` (${c.focus})` : ""}`).join("، ")}",
